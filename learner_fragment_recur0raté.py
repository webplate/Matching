...
def build_patt(sample, w, p=[], before=True):
    #condition d'arrêt
    if w == "":
        return []
    l = len(w)
    ...
    for k in range (l, 0, -1) :
        for i in range(0,l-k+1) :
            ...
            if before :
                ...
                np =  p + [cand]
            print np, match_sample(sample, np)
            if match_sample(sample, np):
                found = True
            if found : break
        if found : break
    if found :
        ...
        ante = w[:i]
        post = w[i+k:]
        print "ante", ante, "post", post, "patt", np
        p_ante = build_patt(sample, ante, np, True)
        p_post = build_patt(sample, post, np, False)
